        ...
        var updateChildrenNodesId = function (node) {
            if (node.children !== undefined && node.children.length > 0) {
                node.children.forEach(function (child) {
                    child.data.id = guid()
                    child.data.parent_id = node.data.id
                        ...
                        if (MimeType.whichMimeType(textData) === 'application/km') {
                            ...
                            sNodes.forEach(function (node) {
                                ...
                                for (var i = nodes.length - 1; i >= 0; i--) {
                                    ...
                                    newNodes.push(_node)
                                    newNodes = newNodes.concat(_node.getDescendants())
                                }
                            })
                            ...
                            newNodes.forEach(function (node) {
                                newNodesData.push(node.getData());
                            });


                            minder.fire('datachange', {resaon: 'paste', datas: newNodesData})
